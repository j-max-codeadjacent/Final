...
def sentences_forDF(title):
	...
	for sent in sent_tokens:
		length = len(sent)
		lc_sent = sent.lower()
		senttokens_bybook.append([title, sent, lc_sent, length])
...
labels = ["Title", 'Sentence', 'LowerCase Sent', 'Sentence Length']
df = pd.DataFrame.from_records(sentencesbybook_forDF, columns = labels)

stemmed_sentences = []


"""df = pd.DataFrame.from_records({'Title': sentencesbybook_forDF[0],
	"Sentences": sentencesbybook_forDF[1],
	})
"""
...
for key in title_keys:
	...
	sentences_forDF[key] = sent_tokens
...
for sent in sentences_forDF:
	lc_sent = sent.lower()
	lcsentences_forDF.append(lc_sent)
	


#Average sentence length includes white spaces and punctuation, including quotation marks.

sentencelengths_forDF = []
for x in sentences_forDF:
	sentencelengths_forDF.append(len(x)) 			
 	
#sentencelengths_forDF = sentencelengths_forDF()


df = pd.DataFrame.from_records({"Sentences": sentences_forDF,
 "Lower Case Sentences": lcsentences_forDF,
 "Sentence Lengths": sentencelengths_forDF,
 })
print df.head()
"""
