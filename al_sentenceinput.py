...
while len(usr_sent) < 12:
	usr_sent = raw_input("Type a sentence longer than 12 characters")

usr_sent = usr_sent.lower()
match = 0
for title in title_keys:
	book = texts[title]
	book = open(book).read().lower().replace('\n', ' ')
	if usr_sent in book:
		print titles[title][0]
		match += 1
