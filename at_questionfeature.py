...
for x in authorwords:
	...
	for sentence in sent_tokens:
		...
		if '?' in sentence:
			question = 1
		sent_byauthor.append([x, authorwords[x][0], sentence, sent_length, question])

df = pd.DataFrame(sent_byauthor)
df.columns = ['book', 'author', 'sentence', 'sentence_length', "Question"]
...
predictors = ['sentence_length', 'Question']
X = df[predictors]
